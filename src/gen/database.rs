...
#[derive(Serialize, Deserialize, Debug, Clone, FromRow)]
pub struct FunUserWhitelistRowType {
    pub total: i64,
    pub expert_id: i64,
    pub user_id: i64,
    pub user_public_id: i64,
...
impl DatabaseRequest for FunUserGetExpertProfileReq {
    type ResponseRow = FunUserExpertRowType;
    fn statement(&self) -> &str {
        "SELECT * FROM api.fun_user_get_expert_profile(a_user_id => $1::bigint, a_expert_id => $2::bigint, a_expert_public_id => $3::bigint);"
    }
    fn params(&self) -> Vec<&(dyn ToSql + Sync)> {
        vec![
            &self.user_id as &(dyn ToSql + Sync),
...
pub struct FunUserListStrategyWalletsReq {
    ...
    pub user_id: Option<i64>,
    #[serde(default)]
    pub strategy_wallet_address: Option<BlockchainAddress>,
    #[serde(default)]
    pub blockchain: Option<EnumBlockChain>,
}
...
impl DatabaseRequest for FunUserListStrategyWalletsReq {
    ...
    fn statement(&self) -> &str {
        "SELECT * FROM api.fun_user_list_strategy_wallets(a_user_id => $1::bigint, a_strategy_wallet_address => $2::varchar, a_blockchain => $3::enum_block_chain);"
    }
    fn params(&self) -> Vec<&(dyn ToSql + Sync)> {
        vec![
            &self.user_id as &(dyn ToSql + Sync),
...
impl DatabaseRequest for FunWatcherListExpertListenedWalletAssetBalanceReq {
    ...
    fn statement(&self) -> &str {
        "SELECT * FROM api.fun_watcher_list_expert_listened_wallet_asset_balance(a_limit => $1::bigint, a_offset => $2::bigint, a_strategy_id => $3::bigint, a_address => $4::varchar, a_blockchain => $5::enum_block_chain, a_token_id => $6::bigint);"
    }
    fn params(&self) -> Vec<&(dyn ToSql + Sync)> {
        vec![
            &self.limit as &(dyn ToSql + Sync),
