...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByUserReq {
    pub user_id: i64,
}
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByUserRespRow {
    pub bucket_id: i64,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByUserResp {
    pub rows: Vec<FunUserListBucketsByUserRespRow>,
}
impl DbClient {
    #[allow(unused_variables)]
    pub async fn fun_user_list_buckets_by_user(
        &self,
        req: FunUserListBucketsByUserReq,
    ) -> Result<FunUserListBucketsByUserResp> {
        let rows = self
            .client
            .query(
                "SELECT * FROM api.fun_user_list_buckets_by_user(a_user_id => $1::bigint);",
                &[&req.user_id],
            )
            .await?;
        let mut resp = FunUserListBucketsByUserResp {
            rows: Vec::with_capacity(rows.len()),
        };
        for row in rows {
            let r = FunUserListBucketsByUserRespRow {
                bucket_id: row.try_get(0)?,
                bucket_name: row.try_get(1)?,
                description: row.try_get(2)?,
                organization_id: row.try_get(3)?,
                always_online: row.try_get(4)?,
            };
            resp.rows.push(r);
        }
        Ok(resp)
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByOrganizationReq {
    pub organization_id: i64,
}
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByOrganizationRespRow {
    pub bucket_id: i64,
    pub bucket_name: String,
    pub description: String,
    pub organization_id: i64,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct FunUserListBucketsByOrganizationResp {
    pub rows: Vec<FunUserListBucketsByOrganizationRespRow>,
}
impl DbClient {
    #[allow(unused_variables)]
    pub async fn fun_user_list_buckets_by_organization(
        &self,
        req: FunUserListBucketsByOrganizationReq,
    ) -> Result<FunUserListBucketsByOrganizationResp> {
        let rows = self.client.query("SELECT * FROM api.fun_user_list_buckets_by_organization(a_organization_id => $1::bigint);", &[&req.organization_id]).await?;
        let mut resp = FunUserListBucketsByOrganizationResp {
            rows: Vec::with_capacity(rows.len()),
        };
        for row in rows {
            let r = FunUserListBucketsByOrganizationRespRow {
                bucket_id: row.try_get(0)?,
                bucket_name: row.try_get(1)?,
                description: row.try_get(2)?,
                organization_id: row.try_get(3)?,
